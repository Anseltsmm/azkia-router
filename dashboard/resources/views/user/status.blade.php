...
<style>
    ...
    .st-card{background:var(--panel);border:1px solid var(--line);border-radius:10px;padding:11px 13px;box-shadow:var(--shadow-card);transition:box-shadow .18s,border-color .18s}
    .st-card:hover{box-shadow:var(--shadow-hover);border-color:var(--line-strong)}
    .st-head{display:flex;align-items:flex-start;gap:10px;flex-wrap:wrap}
    .st-dot{width:9px;height:9px;border-radius:50%;margin-top:4px;flex:0 0 auto}
    ...
    .st-name strong{font-size:13px;font-weight:750;letter-spacing:-.01em;word-break:break-all}
    .st-name .up{font-size:10.5px;color:var(--muted);font-family:ui-monospace,Menlo,Consolas,monospace;font-weight:400}
    .st-badges{display:flex;gap:4px;margin-top:4px;flex-wrap:wrap;align-items:center}
    .st-badges .badge svg{width:10px;height:10px;display:block}
    .st-badges .badge{padding:2px 6px;font-size:10.5px}
    .st-stats{display:flex;gap:16px;flex-shrink:0;padding-top:2px}
    .st-num .lbl{font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:var(--muted)}
    .st-num .val{font-size:13.5px;font-weight:800;font-variant-numeric:tabular-nums;letter-spacing:-.01em}
    .st-num .sub{display:block;font-size:10px;color:var(--muted);margin-top:1px}

    .tl{margin-top:8px;padding-top:6px;border-top:1px dashed var(--line)}
    ...
    .tl-legend{display:flex;align-items:center;gap:10px;font-size:11px;color:var(--muted);flex-wrap:wrap}
    ...
    @media (max-width:639.98px){
        ...
        .st-card{padding:9px 11px}
        .st-head{flex-direction:column;align-items:stretch;gap:6px}
...
<div class="top">
    <div><h2>{{ __('dashboard.titles.status') }}</h2><p>{{ __('dashboard.pages.status.subtitle') }}</p></div>
    <span class="pill">{{ __('dashboard.pages.status.active_models', ['count' => $models->count()]) }}</span>
</div>
...
<div class="st-hero {{ $overall[0] }}">
    ...
    <div>
        <h2>{{ $overall[1] }}</h2>
        <p>{{ __('dashboard.pages.status.summary', ['models' => $models->count(), 'requests' => number_format($totalRequests), 'errors' => $totalErrors]) }}</p>
    </div>
    <div class="right">
        <div class="check">
            <strong>{{ $liveCheckedAt ? \Illuminate\Support\Carbon::parse($liveCheckedAt)->locale(app()->getLocale())->translatedFormat('d M Y H:i:s') : '—' }}</strong>
            {{ __('dashboard.pages.status.last_check') }}
        </div>
    </div>
</div>

@if($liveError)
    <div style="background:var(--red-soft);border:1px solid var(--red-line);color:var(--red-ink);border-radius:10px;padding:10px 14px;margin-bottom:14px;font-size:13px">⚠️ {{ __('dashboard.pages.status.live_unavailable', ['error' => $liveError]) }}</div>
@endif
...
<div class="st-toolbar">
    <div class="st-search">
        ...
        <input id="st-search" type="search" placeholder="{{ __('dashboard.pages.status.search_placeholder') }}" autocomplete="off" aria-label="{{ __('dashboard.pages.models.search_label') }}">
    </div>
    <div class="st-chips" role="group" aria-label="{{ __('dashboard.pages.status.filter') }}">
        <button class="chip active" data-st="all" aria-pressed="true">{{ __('dashboard.common.all') }} <span class="n"></span></button>
        <button class="chip" data-st="Operational" aria-pressed="false">{{ __('dashboard.status.operational') }} <span class="n"></span></button>
        <button class="chip" data-st="Degraded" aria-pressed="false">{{ __('dashboard.status.degraded') }} <span class="n"></span></button>
        <button class="chip" data-st="Down" aria-pressed="false">{{ __('dashboard.status.down') }} <span class="n"></span></button>
        <button class="chip" data-st="No pricing" aria-pressed="false">{{ __('dashboard.status.no_pricing') }} <span class="n"></span></button>
    </div>
</div>
...
<div class="st-list">
    @forelse($models as $model)
        @php
            ...
            if (! $hasPricing) { $stColor = 'red'; $stLabel = 'No pricing'; }
            ...
            $latSub = $s ? __('dashboard.pages.status.average_usage') : __('dashboard.pages.status.not_used');
            $uptimeSub = $requests > 0 ? number_format($requests).' '.strtolower(__('dashboard.common.requests')) : __('dashboard.pages.status.no_requests');
        @endphp
        <div class="st-card" data-status="{{ $stLabel }}" data-name="{{ strtolower($model->public_name) }}">
            <div class="st-head">
                <span class="st-dot {{ $stColor }}" title="{{ __('dashboard.status.'.str_replace(' ', '_', strtolower($stLabel))) }}"></span>
                <div class="st-name">
                    @php
                    ...
                    <div class="st-badges">
                        ...
                        @endforeach
                        <span class="badge {{ $stColor === 'green' ? 'green' : ($stColor === 'red' ? 'red' : ($stColor === 'amber' ? 'amber' : '')) }}">{{ __('dashboard.status.'.str_replace(' ', '_', strtolower($stLabel))) }}</span>
                        @if($pingLatency !== null)<span class="badge {{ $pingOk ? 'green' : 'red' }}">{{ $pingOk ? '●' : '✕' }} {{ $latStr }}</span>@endif
                    </div>
                </div>
                <div class="st-stats">
                    <div class="st-num">
                        <span class="lbl">{{ __('dashboard.pages.status.uptime') }}</span>
                        <div class="val">{{ $uptime !== null ? $uptime.'%' : '—' }}</div>
                        <span class="sub">{{ $uptimeSub }}</span>
                    </div>
                    <div class="st-num">
                        <span class="lbl">{{ __('dashboard.pages.status.latency') }}</span>
            ...
            <div class="tl">
                <div class="tl-scroll">
                    <div class="tl-cells" aria-label="{{ __('dashboard.pages.status.history') }}">
                        @foreach($days as $day)
                            @php
                                $d = $daily->get($model->public_name, collect())->firstWhere('bucket', $day->format('Y-m-d H:i'));
                                $cell = 'gray';
    ...
    @empty
        <div class="card" style="text-align:center;padding:44px 24px">
            <h3 style="margin:0 0 4px;font-size:15px">{{ __('dashboard.pages.status.no_active_heading') }}</h3>
            <p style="margin:0;color:var(--muted);font-size:13.5px">{{ __('dashboard.pages.status.no_active_text') }}</p>
        </div>
    @endforelse
</div>

<div id="st-empty" style="display:none;text-align:center;padding:32px 16px;color:var(--muted);font-size:13.5px">{{ __('dashboard.pages.status.empty_filter') }}</div>

<div class="tl-legend" style="margin-top:14px">
    <span>{{ __('dashboard.pages.status.legend') }}</span>
    <span style="margin-left:auto"><span><i class="g"></i>{{ __('dashboard.pages.status.legend_ok') }}</span><span><i class="a"></i>{{ __('dashboard.pages.status.legend_error') }}</span><span><i class="r"></i>{{ __('dashboard.pages.status.legend_down') }}</span><span><i class="n"></i>{{ __('dashboard.pages.status.legend_no_request') }}</span></span>
</div>
...
(function () {
    ...
    // Muat ulang otomatis setiap 10 menit agar status tetap segar.
