...
<div class="auth-bg">
    <div class="auth-card">
        ...
        <div class="auth-top">
            ...
            <p>{{ $isAdmin ? 'Masuk ke panel admin Azkia Router' : 'Masuk ke Azkia Router control panel' }}</p>
        </div>
        @if($errors->any())<div class="error">{{ $errors->first() }}</div>@endif
        <form method="post" action="{{ route($isAdmin ? 'admin.login.store' : 'login.store') }}">
            @csrf
            <label class="muted">Email</label>
            <input name="email" type="email" placeholder="{{ $isAdmin ? 'admin@azkia.cloud' : 'nama@email.com' }}" value="{{ old('email') }}" required autofocus>
            <label class="muted">Password</label>
            <input name="password" type="password" placeholder="••••••••" required>
            <button class="btn">Login</button>
        </form>
        @if(!$isAdmin && config('services.google.client_id'))
        ...
        @endif
        @if(!$isAdmin)
        <p class="auth-switch" style="margin-top:16px">Baru pertama kali? Daftar cepat lewat <strong>Google</strong> di atas. Registrasi email dan password dinonaktifkan.</p>
        @endif
    </div>
</div>
@endsection
